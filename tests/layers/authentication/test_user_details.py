...
class TestUserDetails:
    ...
    def test_success(self, mock_cognito_client, mock_logger):
        ...
        mock_response = {
            "UserAttributes": [
                ...
                {"Name": "email_verified", "Value": expected_attributes["email_verified"]}
            ]
        }
        ...
        mock_logger.info.assert_called_once_with(f"Fetched attributes for user: {username}.")
        mock_boto3_client.assert_called_once_with("cognito-idp", region_name=TEST_AWS_REGION)
    ...
    def test_cognito_exception(self, mock_cognito_client, mock_logger):
        ...
        mock_logger.exception.assert_called_once_with(f"Failed to fetch user {username} from Cognito")
        mock_cognito_client.admin_get_user.assert_called_once_with(
            UserPoolId=TEST_USER_POOL_ID,
            Username=username
        )
